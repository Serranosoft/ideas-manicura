...
function validEntry(entry, now) {
    if (!entry || !Number.isFinite(entry.fetchedAt) || entry.fetchedAt > now || now - entry.fetchedAt >= MAX_AGE_MS) return null;
    ...
    return {
        async read() {
            try {
                ...
                return raw && raw.length <= MAX_CATALOG_BYTES + 200 ? validEntry(JSON.parse(raw), now()) : null;
            } catch {
                return null;
            }
        },
...
module.exports = { createAffiliateCatalogCache, validEntry, CACHE_KEY, TTL_MS, MAX_AGE_MS };
