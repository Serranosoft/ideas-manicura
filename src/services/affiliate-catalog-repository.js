...
function createAffiliateCatalogRepository({ storage, url, fetchImpl = (...args) => fetch(...args), now = Date.now, timeoutMs = 5000 }) {
    ...
    async function refresh() {
        ...
        refreshing = (async () => {
            ...
            try {
                ...
                // The affiliate catalog must never prevent the rest of the app from working.
            } finally {
                clearTimeout(timer);
            }
            return snapshot();
    ...
    async function getProductsForMarket(market) {
        return createProductResolver(await getCatalog()).getProductsForMarket(market);
    }

    return { getCatalog, refresh, getProductsForMarket };
}
